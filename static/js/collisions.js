...
/**
 * Resolve a collision between two AI entities, updating scoreGains and aisToRemove.
 * Returns true if ai1 was consumed (caller should break inner loop).
 */
function resolveAICollision(ai1, ai2, i, j, ai1Size, ai2Size, scoreGains, aisToRemove) {
...
export function handleAIAICollisions() {
    ...
    for (let i = 0; i < gameState.aiPlayers.length; i++) {
        ...
        for (let j = i + 1; j < gameState.aiPlayers.length; j++) {
            ...
            const ai1Consumed = resolveAICollision(ai1, ai2, i, j, ai1Size, ai2Size, scoreGains, aisToRemove);
