...
function resolveAIPairCollision(ai1, ai2, i, j, scoreGains, aisToRemove) {
...
function isValidAI(ai) {
    return ai && typeof ai.score === 'number';
}
...
export function handleAIAICollisions() {
    ...
    for (let i = 0; i < gameState.aiPlayers.length; i++) {
        ...
        if (!isValidAI(ai1)) continue;

        for (let j = i + 1; j < gameState.aiPlayers.length; j++) {
            if (aisToRemove.has(j)) continue;
            const ai2 = gameState.aiPlayers[j];
            if (!isValidAI(ai2)) continue;

            const consumed = resolveAIPairCollision(ai1, ai2, i, j, scoreGains, aisToRemove);
            if (consumed) break;
        }
    }
