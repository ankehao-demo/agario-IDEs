...
function processPlayerAIPair(playerCell, playerCellIndex, ai, aiIndex, state) {
    if (state.aiIndicesToRemove.has(aiIndex)) return;
    if (state.playerCellsToRemove.has(playerCellIndex)) return;
    ...
    if (playerSize > aiSize * COLLISION_THRESHOLD) {
        const currentGain = state.scoreGains.get(playerCellIndex) || 0;
        state.scoreGains.set(playerCellIndex, currentGain + ai.score + 100);
        state.aiIndicesToRemove.add(aiIndex);
    } else if (aiSize > playerSize * COLLISION_THRESHOLD) {
        // Prevent score overflow
        ai.score = Math.min(Number.MAX_SAFE_INTEGER, ai.score + playerCell.score + 100);
        state.playerCellsToRemove.add(playerCellIndex);
    }
}

function removeIndicesDescending(arr, indicesSet) {
    [...indicesSet].sort((a, b) => b - a).forEach(index => {
        arr.splice(index, 1);
    });
}

function applyPlayerScoreGains(scoreGains, playerCellsToRemove) {
    scoreGains.forEach((gain, cellIndex) => {
        if (playerCellsToRemove.has(cellIndex)) return;
        const cell = gameState.playerCells[cellIndex];
        if (!cell) return;
        // Prevent score overflow
        cell.score = Math.min(Number.MAX_SAFE_INTEGER, cell.score + gain);
    });
}

function respawnPlayerIfDead() {
...
export function handlePlayerAICollisions() {
    ...
    gameState.playerCells.forEach((playerCell, playerCellIndex) => {
        ...
        gameState.aiPlayers.forEach((ai, aiIndex) => {
            ...
            processPlayerAIPair(playerCell, playerCellIndex, ai, aiIndex, state);
        });
    });

    removeIndicesDescending(gameState.aiPlayers, state.aiIndicesToRemove);
    applyPlayerScoreGains(state.scoreGains, state.playerCellsToRemove);
    removeIndicesDescending(gameState.playerCells, state.playerCellsToRemove);
    respawnPlayerIfDead();
}
