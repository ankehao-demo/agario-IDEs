...
function checkPlayerAICollision(playerCell, playerCellIndex, ai, aiIndex, results) {
    if (results.aiIndicesToRemove.has(aiIndex)) return;
    if (results.playerCellsToRemove.has(playerCellIndex)) return;
    ...
    if (playerSize > aiSize * COLLISION_THRESHOLD) {
        const currentGain = results.scoreGains.get(playerCellIndex) || 0;
        results.scoreGains.set(playerCellIndex, currentGain + ai.score + 100);
        results.aiIndicesToRemove.add(aiIndex);
    } else if (aiSize > playerSize * COLLISION_THRESHOLD) {
        ai.score = Math.min(Number.MAX_SAFE_INTEGER, ai.score + playerCell.score + 100);
        results.playerCellsToRemove.add(playerCellIndex);
    }
}

function applyCollisionResults(results) {
    [...results.aiIndicesToRemove].sort((a, b) => b - a).forEach(index => {
        gameState.aiPlayers.splice(index, 1);
    });

    results.scoreGains.forEach((gain, cellIndex) => {
        if (!results.playerCellsToRemove.has(cellIndex) && gameState.playerCells[cellIndex]) {
            gameState.playerCells[cellIndex].score = Math.min(Number.MAX_SAFE_INTEGER, gameState.playerCells[cellIndex].score + gain);
        }
    });

    [...results.playerCellsToRemove].sort((a, b) => b - a).forEach(index => {
...
export function handlePlayerAICollisions() {
    ...
    gameState.playerCells.forEach((playerCell, playerCellIndex) => {
        ...
        gameState.aiPlayers.forEach((ai, aiIndex) => {
            ...
            checkPlayerAICollision(playerCell, playerCellIndex, ai, aiIndex, results);
        });
    });
