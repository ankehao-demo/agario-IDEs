...
function minDistanceTo(pos, entities) {
    return entities.reduce((min, entity) => Math.min(min, getDistance(pos, entity)), Infinity);
}
...
export function findSafeSpawnLocation(gameState, minDistance = 100) {
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
        ...
        if (isSafeFrom(pos, entities, minDistance)) {
            return pos;
        }
    }
    ...
    for (let i = 0; i < 20; i++) {
        const pos = getRandomPosition();
        const minDistanceToPlayer = minDistanceTo(pos, entities);
