...
function handleMergeableClose(cell1, cell2, i, j, distance, minDistance, cellsToMerge) {
...
function handleNonMergeable(cell1, cell2, distance, minDistance, canMerge) {
...
function processCellPairForces(cellsToMerge) {
    const now = Date.now();
    ...
    for (let i = 0; i < gameState.playerCells.length; i++) {
        ...
        for (let j = i + 1; j < gameState.playerCells.length; j++) {
            ...
            if (distance < minMergeDistance && canMerge) {
                handleMergeableClose(cell1, cell2, i, j, distance, minDistance, cellsToMerge);
            } else {
                handleNonMergeable(cell1, cell2, distance, minDistance, canMerge);
            }
        }
    }
}
...
function updateCellMerging() {
    const cellsToMerge = [];
    processCellPairForces(cellsToMerge);
