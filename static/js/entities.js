...
// Applies attraction/repulsion between two cells and returns true if they should merge.
...
// Groups descending-sorted indices into runs of consecutive values.
...
function updateCellMerging() {
    ...
    for (let i = 0; i < cells.length; i++) {
        if (!isValidCell(cells[i]) || cellsToMerge.has(i)) continue;

        for (let j = i + 1; j < cells.length; j++) {
            if (!isValidCell(cells[j]) || cellsToMerge.has(j)) continue;
    ...
    if (cellsToMerge.size === 0) return;

    // Second pass: merge cells, highest indices first so splices stay valid
    const sortedIndices = [...cellsToMerge].sort((a, b) => b - a);
    groupConsecutiveIndices(sortedIndices).forEach(mergeCellGroup);
}
