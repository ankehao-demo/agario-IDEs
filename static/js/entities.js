...
function applyGeneralAttraction(cell1, cell2, distance, canMerge) {
    const dx = cell2.x - cell1.x;
    const dy = cell2.y - cell1.y;
...
function processCellPairForces(cell1, cell2, i, j, cellsToMerge, now) {
    ...
    if (distance > minDistance) {
        applyGeneralAttraction(cell1, cell2, distance, canMerge);
    }
}

function buildMergeGroups(cellsToMerge) {
...
function updateCellMerging() {
    ...
    if (cellsToMerge.length > 0) {
        buildMergeGroups(cellsToMerge).forEach(mergeGroup);
    }
}
