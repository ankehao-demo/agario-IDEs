...
function applySymmetricForce(cell1, cell2, distance, forceMagnitude) {
    const dx = cell2.x - cell1.x;
    const dy = cell2.y - cell1.y;
    const factor = forceMagnitude / Math.max(1, distance);
...
function applyRepulsion(cell1, cell2, distance, minDistance) {
...
function handleMergeablePair(cell1, cell2, distance, minDistance, canMerge, cellsToMerge, i, j) {
    if (distance < minDistance * 0.5) {
        cellsToMerge.push(i, j);
    } else {
        applySymmetricForce(cell1, cell2, distance, MERGE_FORCE);
    }
}

function handleNonMergeablePair(cell1, cell2, distance, minDistance, canMerge) {
    if (distance < minDistance) {
        applyRepulsion(cell1, cell2, distance, minDistance);
    }
    if (distance > minDistance) {
        const force = canMerge ? MERGE_FORCE : MERGE_START_FORCE;
        applySymmetricForce(cell1, cell2, distance, force);
    }
}

function groupMergeIndices(cellsToMerge) {
...
function performMerge(cellsToMerge) {
    const groups = groupMergeIndices(cellsToMerge);
...
function isValidCell(cell) {
    return cell && typeof cell.score === 'number';
}
    ...
    for (let i = 0; i < gameState.playerCells.length; i++) {
        ...
        for (let j = i + 1; j < gameState.playerCells.length; j++) {
            ...
            if (distance < minMergeDistance && canMerge) {
                handleMergeablePair(cell1, cell2, distance, minDistance, canMerge, cellsToMerge, i, j);
            } else {
                handleNonMergeablePair(cell1, cell2, distance, minDistance, canMerge);
            }
        }
    }

    if (cellsToMerge.length > 0) {
        performMerge(cellsToMerge);
    }
}
