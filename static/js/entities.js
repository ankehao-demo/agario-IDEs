...
function applyMergeAttraction(cell1, cell2, distance) {
...
function applyAttraction(cell1, cell2, distance, canMerge) {
    const dx = cell2.x - cell1.x;
    const dy = cell2.y - cell1.y;
...
function processCellPairForces(cell1, cell2, cellsToMerge, i, j, now) {
    ...
    if (distance < minMergeDistance && canMerge) {
        if (distance < minDistance * 0.5) {
            ...
            applyMergeAttraction(cell1, cell2, distance);
        }
        return;
    }
    ...
    if (distance > minDistance) {
        applyAttraction(cell1, cell2, distance, canMerge);
    }
}
...
function updateCellMerging() {
    ...
    for (let i = 0; i < gameState.playerCells.length; i++) {
        ...
        for (let j = i + 1; j < gameState.playerCells.length; j++) {
            ...
            processCellPairForces(cell1, cell2, cellsToMerge, i, j, now);
        }
    }
