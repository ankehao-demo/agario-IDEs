...
// Pushes cell1 and cell2 towards (positive factor) or away from (negative factor) each other
...
// Returns true if the pair should be merged, otherwise applies forces between them
...
function findCellsToMerge(now) {
    ...
    for (let i = 0; i < cells.length; i++) {
        ...
        for (let j = i + 1; j < cells.length; j++) {
            ...
            if (processCellPair(cells[i], cells[j], now)) {
...
// Groups descending-sorted unique indices into runs of consecutive indices
...
function updateCellMerging() {
    const cellsToMerge = findCellsToMerge(Date.now());
    if (cellsToMerge.length === 0) return;

    // Sort indices in descending order so removal happens from the end first
