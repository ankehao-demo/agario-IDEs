...
// Apply an attraction/repulsion force between two cells.
// A positive factor attracts cell2 toward cell1; a negative one repels them.
...
// Resolve the interaction between a pair of cells, either marking them for
// merging or nudging them together/apart. Returns true if the pair was marked.
function resolveCellPair(cell1, cell2, i, j, now, cellsToMerge) {
...
// Group sorted unique indices into runs of consecutive values.
...
// Combine a group of cells into a single mass-weighted cell.
...
function updateCellMerging() {
    ...
    for (let i = 0; i < gameState.playerCells.length; i++) {
        ...
        for (let j = i + 1; j < gameState.playerCells.length; j++) {
            ...
            resolveCellPair(cell1, cell2, i, j, now, cellsToMerge);
        }
    }
