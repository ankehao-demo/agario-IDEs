...
function applySymmetricForce(cell1, cell2, dx, dy, factor) {
...
function applyMergeAttraction(cell1, cell2, distance) {
    ...
    applySymmetricForce(cell1, cell2, dx, dy, factor);
}
...
function applyRepulsion(cell1, cell2, distance, minDistance) {
    ...
    const dx = cell2.x - cell1.x;
    const dy = cell2.y - cell1.y;
    applySymmetricForce(cell1, cell2, dx, dy, -repulsionFactor);
}
...
function updateCellMerging() {
    ...
    if (cellsToMerge.length > 0) {
        const groups = buildMergeGroups(cellsToMerge);
