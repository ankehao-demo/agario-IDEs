...
function applyForceBetweenCells(cell1, cell2, dx, dy, factor) {
...
function applyRepulsion(cell1, cell2, distance, minDistance) {
    ...
    applyForceBetweenCells(cell1, cell2, -dx * repulsionFactor, -dy * repulsionFactor, 1);
}
...
function applyAttraction(cell1, cell2, distance, force) {
    ...
    applyForceBetweenCells(cell1, cell2, dx, dy, factor);
}

function processCellPairPhysics(cell1, cell2, i, j, now, cellsToMerge) {
...
function performCellMerge(cellsToMerge) {
...
function updateCellMerging() {
    ...
    for (let i = 0; i < gameState.playerCells.length; i++) {
        ...
        for (let j = i + 1; j < gameState.playerCells.length; j++) {
            ...
            processCellPairPhysics(cell1, cell2, i, j, now, cellsToMerge);
        }
    }

    if (cellsToMerge.length > 0) {
        performCellMerge(cellsToMerge);
    }
}
