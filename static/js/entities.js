...
// Apply a directional force between two cells
function applyForceBetweenCells(cell1, cell2, distance, forceMagnitude) {
...
// Apply repulsion force when cells are too close but can't merge
...
// Handle interaction between a pair of cells (merge marking, attraction, or repulsion)
function processCellPair(cell1, cell2, i, j, now, cellsToMerge) {
    ...
    if (distance < minMergeDistance && canMerge) {
        if (distance < minDistance * 0.5) {
            ...
            applyForceBetweenCells(cell1, cell2, distance, MERGE_FORCE);
        }
        return;
    }
    ...
    if (distance > minDistance) {
        ...
        applyForceBetweenCells(cell1, cell2, distance, force);
    }
}
