...
function applyForce(cell1, cell2, dx, dy, factor) {
...
function handleMergeableClose(cell1, cell2, distance, minDistance, cellsToMerge, i, j) {
    ...
    applyForce(cell1, cell2, dx, dy, factor);
}

function handleRepulsion(cell1, cell2, distance, minDistance) {
    const repulsionStrength = 0.3;
    const repulsionFactor = (minDistance - distance) / minDistance * repulsionStrength;
    const dx = cell2.x - cell1.x;
    const dy = cell2.y - cell1.y;
    applyForce(cell1, cell2, -dx, -dy, repulsionFactor);
}

function handleAttraction(cell1, cell2, distance, canMerge) {
    const dx = cell2.x - cell1.x;
    const dy = cell2.y - cell1.y;
...
function processCellPair(cell1, cell2, i, j, now, cellsToMerge) {
    ...
    if (distance < minMergeDistance && canMerge) {
        handleMergeableClose(cell1, cell2, distance, minDistance, cellsToMerge, i, j);
        return;
    }

    if (distance < minDistance) {
        handleRepulsion(cell1, cell2, distance, minDistance);
    }
    if (distance > minDistance) {
        handleAttraction(cell1, cell2, distance, canMerge);
    }
}

function groupCellIndices(uniqueIndices) {
    const groups = [];
...
function mergeGroup(group) {
    ...
    [...group].toSorted((a, b) => b - a).forEach(index => {
...
function performMerges(cellsToMerge) {
    if (cellsToMerge.length === 0) return;
    
    cellsToMerge.sort((a, b) => b - a);
    const uniqueIndices = [...new Set(cellsToMerge)];
    const groups = groupCellIndices(uniqueIndices);
    groups.forEach(mergeGroup);
}
