...
function resolveCellPair(cell1, cell2, now) {
    ...
    if (distance < minMergeDistance && canMerge) {
        ...
        applyVelocity(cell1, cell2, dx, dy, factor);
    } else {
...
function groupMergeIndices(uniqueIndices) {
    const groups = [];
