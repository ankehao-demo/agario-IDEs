...
function isMergeCandidate(cell, index, cellsToMerge) {
    return Boolean(cell) && typeof cell.score === 'number' && !cellsToMerge.includes(index);
}

// Apply an attraction/repulsion velocity change between two cells.
// A positive force attracts cell1 towards cell2; a negative force repels.
...
function mergeMarkedCells(cellsToMerge) {
    // Unique indices in descending order to remove from end first
