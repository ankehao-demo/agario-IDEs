...
function applyRepulsion(cell1, cell2, distance, minDistance) {
    ...
    applySymmetricForce(cell1, cell2, -dx * repulsionFactor, -dy * repulsionFactor, 1);
}

function applyAttraction(cell1, cell2, distance, force) {
    const dx = cell2.x - cell1.x;
    const dy = cell2.y - cell1.y;
...
function processCellPair(cell1, cell2, i, j, cellsToMerge, now) {
    ...
    if (distance < minMergeDistance && canMerge) {
        if (distance < minDistance * 0.5) {
            ...
            applyAttraction(cell1, cell2, distance, MERGE_FORCE);
        }
        return;
    }
    ...
    if (distance > minDistance) {
        ...
        applyAttraction(cell1, cell2, distance, force);
    }
}

function executeCellMerges(cellsToMerge) {
...
function updateCellMerging() {
    ...
    if (cellsToMerge.length > 0) {
        executeCellMerges(cellsToMerge);
    }
}
