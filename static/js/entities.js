...
function handleMergeablePair(cell1, cell2, i, j, distance, minDistance, cellsToMerge) {
...
function mergeGroupIntoCells(group) {
...
function updateCellMerging() {
    ...
    for (let i = 0; i < gameState.playerCells.length; i++) {
        ...
        for (let j = i + 1; j < gameState.playerCells.length; j++) {
            ...
            if (distance < minMergeDistance && canMerge) {
                handleMergeablePair(cell1, cell2, i, j, distance, minDistance, cellsToMerge);
            } else {
    ...
    if (cellsToMerge.length > 0) {
        buildMergeGroups(cellsToMerge).forEach(mergeGroupIntoCells);
    }
}
