...
function applyAttractionForce(cell1, cell2, dx, dy, distance, force) {
...
function applyRepulsionForce(cell1, cell2, dx, dy, distance, minDistance) {
...
function processMergePair(cell1, cell2, now, cellsToMerge, i, j) {
    ...
    if (distance < minMergeDistance && canMerge) {
        if (distance < minDistance * 0.5) {
            ...
            applyAttractionForce(cell1, cell2, dx, dy, distance, MERGE_FORCE);
        }
        return;
    }

    if (distance < minDistance) {
        applyRepulsionForce(cell1, cell2, dx, dy, distance, minDistance);
    }

    if (distance > minDistance) {
        const force = canMerge ? MERGE_FORCE : MERGE_START_FORCE;
        applyAttractionForce(cell1, cell2, dx, dy, distance, force);
    }
}
...
function applyMerges(cellsToMerge) {
...
function updateCellMerging() {
    ...
    for (let i = 0; i < gameState.playerCells.length; i++) {
        ...
        for (let j = i + 1; j < gameState.playerCells.length; j++) {
            ...
            processMergePair(cell1, cell2, now, cellsToMerge, i, j);
        }
    }

    // Second pass: merge cells
    applyMerges(cellsToMerge);
}
