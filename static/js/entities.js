...
function applyForce(cell1, cell2, distance, forceMagnitude) {
...
function handleMergeablePair(cell1, cell2, distance, minDistance, cellsToMerge, i, j) {
    if (distance < minDistance * 0.5) {
        cellsToMerge.push(i, j);
    } else {
        applyForce(cell1, cell2, distance, MERGE_FORCE);
    }
}
...
function handleNonMergeablePair(cell1, cell2, distance, minDistance, canMerge) {
    ...
    if (distance > minDistance) {
        ...
        applyForce(cell1, cell2, distance, force);
    }
}

function processCellPair(cell1, cell2, i, j, cellsToMerge, now) {
    ...
    if (distance < minMergeDistance && canMerge) {
        handleMergeablePair(cell1, cell2, distance, minDistance, cellsToMerge, i, j);
    } else {
        handleNonMergeablePair(cell1, cell2, distance, minDistance, canMerge);
    }
}

function executeCellMerge(cellsToMerge) {
...
function updateCellMerging() {
    ...
    for (let i = 0; i < gameState.playerCells.length; i++) {
        ...
        for (let j = i + 1; j < gameState.playerCells.length; j++) {
            ...
            processCellPair(cell1, cell2, i, j, cellsToMerge, now);
        }
    }

    executeCellMerge(cellsToMerge);
}
